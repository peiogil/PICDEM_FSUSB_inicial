...
typedef enum
{
    ...
    COMMAND_FIN_CUENTA_TIMER0=0X85,        
    ...
    {   
        ...
        //Cuando se pulsa el botón S3 desde el PC en respuesta enciende el Led D4
            ...
            case COMMAND_A_CONTAR_TIMER0:   //Cuando recibe una orden del PC cuenta un tiempo programado en el TIMER0.
                ...
                TIMER0_CONTANDO=true;
                }
            break;
            case COMMAND_FIN_CUENTA_TIMER0:
                {
                //Check to make sure the endpoint/buffer is free before we modify the contents
                if(!HIDTxHandleBusy(USBInHandle))
                {
                    ToSendDataBuffer[0] = 0x85;				//Echo back to the host PC the command we are fulfilling in the first uint8_t.
                    ToSendDataBuffer[2] = timerCounter;                                        //In this case, the end of the count of the TIMER0.
                    if(TIMER0_CONTANDO == true)              //Denboragailua aritzen ari den bitartean
                    {
                            ToSendDataBuffer[1] = 0x01;
                            
                    }
                    else									//Denboragailua bukatu ondoren
                    {
                            ToSendDataBuffer[1] = 0x00;
                    }
                    //Prepare the USB module to send the data packet to the host
